...
import security.{UserDetailsService, UserDetailsServiceImpl, VkSSOEnv}
...
class SignInController @Inject()(controllerComponents: ControllerComponents,
                                  ...
                                  userDetailsService: UserDetailsService)(implicit ex: ExecutionContext) extends AbstractController(controllerComponents) {
  ...
  def login = silhouette.UnsecuredAction.async { implicit request: Request[AnyContent] =>
    vKProvider.authenticate().flatMap {
      ...
      case Right(info) => (for {
        ...
        user <- userDetailsService.save(profile, info.accessToken)
